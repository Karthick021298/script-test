  ...
  useEffect(() => {
    if (bot_cheadId && ready) {
      const ws = new ReconnectingWebSocket(
        `${process.env.NEXT_PUBLIC_SOCKET_CONNECTION + bot_cheadId}`
      );
      ws.debug = true;
    ...
    if (socket) {
      socket.onmessage = function (event) {
        ...
        try {
          if (
            (decryptedJson.event =
              "data" && decryptedJson?.chead_id === bot_cheadId)
          ) {
    ...
    <>
      ...
      {showButton && (
        ...
        >
          ...
          <Image
            src={"https://ik.imagekit.io/LyfngoDev/Microsite/Floating_icon.svg"}
            alt="micro-bot"
            width={40}
            height={40}
            priority
