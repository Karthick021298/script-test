import React, { useCallback, useEffect, useState } from "react";
import { Button } from "@mui/material";
import { motion } from "framer-motion";
import { themeConfig } from "@/theme/themesConfig";
import BotApi from "@/service/BotApi";
import moment from "moment";
import secureLocalStorage from "react-secure-storage";
import { IconButton, Typography, makeStyles } from "@material-ui/core";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import useAuth from "@/Utils/Hooks/useAuth";
const useStyles = makeStyles((theme) => ({
  icon: {
    "& .MuiIconButton-root": {
      "& .MuiSvgIcon-root": {
        color: themeConfig.palette.lyfngo.primary.main,
      },
    },
  },
}));
const TimeSlot = (props) => {
  const {
    setMessages,
    availSlot,
    bookModes,
    selectedDate,
    setSelectedDate,
    domainData,
    setCurrentState,
    currentState,
    setLoading,
    userData,
    selectedUser,
    apptDetails,
    endChat,
    messages,
    index,
    data,
  } = props;
  const { bot_custUuid, aiSetStatus, setAICount, custUuid } = useAuth();
  const classes = useStyles();
  const getCurrentTimeIn12HrFormat = () => {
    const now = new Date();
    let hours = now.getHours();
    let minutes = now.getMinutes();
    const amOrPm = hours >= 12 ? "PM" : "AM";
    hours = hours % 12 || 12;
    minutes = minutes < 10 ? "0" + minutes : minutes;
    return `${hours}:${minutes} ${amOrPm}`;
  };

  const formatTo12Hour = (time) => {
    const [hours, minutes] = time.split(":").map(Number);
    const period = hours >= 12 ? "PM" : "AM";
    const hour12 = hours % 12 || 12;
    let result = `${hour12}:${minutes.toString().padStart(2, "0")} ${period}`;
    return moment(result, "h:mm A").format("hh:mm A");
  };
  let filteredSlots =
    availSlot &&
    availSlot?.filter((slot) => {
      const currentTime = getCurrentTimeIn12HrFormat();
      const currentHours = parseInt(currentTime.split(":")[0]);
      const currentMinutes = parseInt(currentTime.split(":")[1].split(" ")[0]);
      const currentAmPm = currentTime.split(" ")[1];
      const slotHours = parseInt(slot.startTime.split(":")[0]);
      const slotMinutes = parseInt(slot.startTime.split(":")[1]);
      const slotAmPm = slot.startTime.split(" ")[1];
      if (currentAmPm === "PM" && slotAmPm === "AM") {
        return true;
      } else if (currentAmPm === "AM" && slotAmPm === "PM") {
        return false;
      } else {
        if (currentHours === slotHours) {
          return currentMinutes < slotMinutes;
        } else {
          return currentHours < slotHours;
        }
      }
    });
  const numberOfSlotsToShow = 15;
  const [currentIndex, setCurrentIndex] = useState(numberOfSlotsToShow);
  const [defaultSlots, setDefaultSlots] = useState(
    filteredSlots.slice(0, numberOfSlotsToShow)
  );
  const seeMore = () => {
    const nextIndex = currentIndex + numberOfSlotsToShow;
    setDefaultSlots(filteredSlots.slice(0, nextIndex));
    setCurrentIndex(nextIndex);
  };
  const lockSlot = useCallback(
    (selectedSlot, slotData) => {
      aiSetStatus(false);
      secureLocalStorage.setItem("AIstatus", false);
      setAICount(0);
      setCurrentState((prevState) => [
        {
          ...prevState[0],
          ChooseChatOption: {
            ...prevState[0]?.ChooseChatOption,
            timeSlotSelected: true,
          },
        },
      ]);
      secureLocalStorage.setItem("AIsentCount", 0);
      let body = {
        apponmintId: "string",
        startDate: selectedDate,
        endDate: selectedDate,
        startTime: slotData?.startTime,
        endTime: moment(slotData?.startTime, "hh:mm A")
          .add(slotData?.intervalInMins, "minutes")
          .format("HH:mm"),
        timezone: domainData?.countryTimezone?.split(" ")?.[2],
        summary: "b2c",
      };
      setMessages((prevMessages) => [
        ...prevMessages,
        {
          name: "CustSlotSelection",
          type: "cust",
          jsonType: "card",
          component: "",
          data: {
            message: selectedSlot,
          },
        },
      ]);
      const onSuccess = (res) => {
        if (res?.data?.status === true) {
          bookDemo(selectedSlot, slotData, res?.data?.data?.meeting_link || "");
        }
      };
      const onFailure = (err) => {
        setMessages((prevMessages) => [
          ...prevMessages,
          {
            name: "meetlinkerror",
            type: "bot",
            jsonType: "card",
            component: "",
            data: {
              message: `Internal Error. Try again with the same once ${selectedSlot} or choose other slots from above.`,
            },
          },
        ]);
      };
      BotApi.googleMeetingLinkGeneration(body, domainData?.mastTentUuid).then(
        onSuccess,
        onFailure
      );
    },
    [selectedDate]
  );
  const onOff = _.isEqual(userData?.appointmentMode || bookModes, "at-clinic")
    ? "0"
    : _.isEqual(userData?.appointmentMode || bookModes, "on-line")
    ? "1"
    : _.isEqual(userData?.appointmentMode || bookModes, "at-home")
    ? "2"
    : null;
  const bookDemo = (selectedSlot, slotData, meetLink) => {
    const body = {
      custId: bot_custUuid || custUuid,
      tentUserId:
        selectedUser?.tentUserDTO?.tentUserUuid || selectedUser?.tentUserUuid,
      tentId: domainData?.mastTentUuid,
      apptCatId: "",
      scheduledOn: userData?.date || selectedDate,
      scheduledTime: slotData?.startTime + ":00+05:30",
      scheduledPeriod: slotData?.intervalInMins,
      scheduledPeriodType: "Mins",
      plannedProcedure: [],
      notes: "",
      onOff: onOff,
      totalAppointmentPrice: slotData?.consultantionFees,
      custMail: true,
      custSms: true,
      tentSms: true,
      tentMail: true,
      type: "BOT",
      appointmentSource: "b2cbot",
      isPaymentMandatory: slotData?.isPaymentMandatory,
      meetingLink: meetLink || null,
    };
    const rescheduleBody = {
      uuid: apptDetails?.app_uuid || apptDetails?.appointmentUuid,
      tentUserId: apptDetails?.tentUserUuid,
      scheduledOn: selectedDate,
      scheduledTime: slotData?.startTime + ":00+05:30",
      custMail: true,
      custSms: true,
      tentSms: true,
      tentMail: true,
      notes: null,
      plannedProcedure: [],
      apptCatId: "",
    };
    setLoading(true);
    const onSuccess = () => {
      setLoading(false);
      setMessages((prevMessages) => [
        ...prevMessages,
        {
          name: "SuccessConfirmation",
          type: "bot",
          jsonType: "card",
          component: "",
          data: {
            message:
              `Successfully ${
                _.isEqual(userData?.endpoint, "reschedule_appointment")
                  ? "rescheduled"
                  : "booked"
              } your appointment on ${selectedDate} at ${selectedSlot} with ${
                _.isEqual(userData?.endpoint, "reschedule_appointment")
                  ? `${
                      apptDetails?.doc_salutation ||
                      apptDetails?.tentUserSalutation
                    } .${
                      apptDetails?.doc_name || apptDetails?.tentUserFirstName
                    }`
                  : userData?.tentUserName ||
                    `${selectedUser?.tentUserDTO?.tentUserSalutation} .${selectedUser?.tentUserDTO?.tentUserName}`
              }, For more information please check your appointment list.` +
              (meetLink && body?.onOff === 1 ? "\n" + meetLink : ""),
          },
        },
        {
          name: "actions",
          type: "bot",
          jsonType: "",
          component: "",
          disabled: false,
          data: [
            {
              actionName: "End Chat",
              id: "endchat",
              onPress: () => {
                endChat();
              },
            },
          ],
        },
      ]);
      setSelectedDate("");
    };
    const onFailure = () => {
      setLoading(true);
      setMessages((prevMessages) => [
        ...prevMessages,
        {
          name: "addApptErr",
          type: "bot",
          jsonType: "card",
          component: "",
          data: {
            message: `Try again with the same once ${selectedSlot} or choose other slots from above.`,
          },
        },
      ]);
    };
    _.isEqual(userData?.endpoint, "reschedule_appointment")
      ? BotApi.rescheduleApp(rescheduleBody).then(onSuccess, onFailure)
      : BotApi.bookDemo(body).then(onSuccess, onFailure);
  };
  const renderTimeSlots = () => {
    return defaultSlots.map((slot, id) => (
      <motion.div
        key={id}
        whileTap={{ scale: 0.95 }}
        style={{
          borderRadius: "10px",
          overflow: "hidden",
          width: "85px",
          height: "32px",
          display: "flex",
          alignSelf: "flex-end",
        }}
      >
        <Button
          key={id}
          sx={{
            width: "85px",
            height: "32px",
            background: "#fff",
            border: `1px solid ${themeConfig.palette.lyfngo.primary.main}`,
            textTransform: "capitalize",
            color: themeConfig.palette.lyfngo.primary.main,
            padding: 0,
            fontFamily: themeConfig.typography.subtitle1.fontFamily,
            "&:hover": {
              background: themeConfig.palette.lyfngo.primary.main,
              border: `1px solid ${themeConfig.palette.lyfngo.primary.main}`,
              color: "#fff",
            },
            fontSize: themeConfig.typography.subtitle1.fontSize,
          }}
          variant="outlined"
          style={{ borderRadius: "10px" }}
          onClick={() => {
            let newArr = _.map(messages, (item, idx) => {
              return idx === index ? { ...item, disabled: true } : item;
            });
            setMessages(newArr);
            lockSlot(formatTo12Hour(slot?.startTime), slot);
          }}
          disabled={data?.disabled}
          id="time"
        >
          {formatTo12Hour(slot?.startTime)}
        </Button>
      </motion.div>
    ));
  };

  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        marginLeft: 30,
        boxShadow: "0px 0px 4px 0px #d9d9d9",
        borderRadius: 10,
      }}
      id="timeslot_selector"
    >
      <div
        style={{
          display: "flex",
          gap: 6,
          paddingTop: 20,
          padding: 10,
          flexWrap: "wrap",
          alignItems: "center",
          justifyContent: "center",
        }}
      >
        {renderTimeSlots()}
      </div>
      {filteredSlots.length > defaultSlots?.length && (
        <div
          style={{ alignItems: "center", justifyContent: "center" }}
          className={classes.icon}
        >
          <IconButton onClick={seeMore}>
            <ExpandMoreIcon />
          </IconButton>
        </div>
      )}
    </div>
  );
};

export default TimeSlot;
